...
<g class='axis x-axis'>
	{#each tickVals as tick, i}
		<g class='tick tick-{ tick }' transform='translate({$xScale(tick)},{$yScale.range()[0]})'>
			...
			<text class='tick' y='16' text-anchor='{textAnchor(i)}'>{formatTick(tick)}</text>
		</g>

		<g  transform='translate({$xScale(tick)},{$yScale.range()[0]})'>
			<line stroke='#111'  y1='4' y2='0' x1='-2' x2='-2'></line>
		</g>
